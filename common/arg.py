...
class TaskArgs:
    ...
    def _create_task_command(self, name: str, help_text: str) -> None:
        ...
        parser.add_argument("-p", "--path", type=str, metavar="<PATH>",
                            default=None, help="Task config directory", dest=f"{name}_path")
        parser.add_argument("-r", "--regex", type=str, metavar="<REGEX>",
                            default=r"^.*\.(toml|ini|json)$", help="Task config filename regex pattern", dest=f"{name}_regex")
        parser.add_argument("-f", "--config", type=str, metavar="<CONFIG>",
                            default=None, help="Task config file", dest=f"{name}_config")
        parser.add_argument("-n", "--name", type=str, metavar="<NAME>",
                            default=None, help="Task name (unique)", dest=f"{name}_name")
        parser.add_argument("-c", "--command", type=str, metavar="<COMMAND>",
                            default=None, help="Task command", dest=f"{name}_command")
        parser.add_argument("-a", "--args", nargs="+", metavar="<ARGS>",
                            default=None, help="Task arguments", dest=f"{name}_args")
        parser.add_argument("-d", "--dir", type=str, metavar="<DIR>",
                            default=None, help="Task working directory", dest=f"{name}_dir")
        parser.add_argument("-e", "--env", nargs="+", metavar="<ENV>",
                            default=None, help="Task environment variables", dest=f"{name}_env")
        parser.add_argument("-i", "--stdin", action="store_true",
                            default=False, help="Task standard input", dest=f"{name}_stdin")
        parser.add_argument("-o", "--stdout", type=str, metavar="<STDOUT>",
                            default=None, help="Task standard output", dest=f"{name}_stdout")
        parser.add_argument("-w", "--stderr", type=str, metavar="<STDERR>",
                            default=None, help="Task standard error", dest=f"{name}_stderr")
    ...
    def _create_flag_command(self, name: str, help_text: str) -> None:
        ...
        parser.add_argument("-i", "--id", type=int, metavar="<ID>",
                            default=None, help="Task id (unique)", dest=f"{name}_id")
        parser.add_argument("-n", "--name", type=str, metavar="<NAME>",
                            default=None, help="Task name (unique)", dest=f"{name}_name")
        parser.add_argument("-m", "--mat", action="store_true",
                            default=False, help="Is match regex pattern by namae", dest=f"{name}_mat")
    ...
    def _create_list_command(self, name: str, help_text: str) -> None:
        ...
        parser.add_argument("-n", "--name", type=str, metavar="<NAME>",
                            default=None, help="Task name (unique)", dest=f"{name}_name")
        parser.add_argument("-m", "--mat", action="store_true",
                            default=False, help="Is match regex pattern by name", dest=f"{name}_mat")
        parser.add_argument("-o", "--more", action="store_true",
                            default=False, help="Show more info", dest=f"{name}_more")
        parser.add_argument("-l", "--less", action="store_true",
                            default=False, help="Show less info", dest=f"{name}_less")
